...
- `src/case/` — the same case, said to a person. `narrative.mjs` turns a finished calculation into five sections of claims, each carrying what it waits on; `projection.mjs` shows that to a role at a depth and may drop anything except a material risk; `specialists.mjs` consults only the ones the case holds enough for; `brief.mjs` drafts the note to a manager; `briefing.mjs` says what changed since last time; `intake.mjs` and `from-quote.mjs` are the ways in. `supply-scene.mjs` is supplier → transport → production → customer *as this case knows them* — a stage with nothing in it says so and names what would fill it, no figure ever appears in it, and a bottleneck is named only when the case's own figures name one. An adopted what-if populates it; an unadopted one never does.
- `src/calc/scenarios.mjs` — the three what-ifs: split the delivery, expedite it, source it elsewhere. A scenario is a copy carrying an explicit badge and the plan revision it was explored against; adopting one records who and when and changes nothing. One missing input withholds every figure, because a comparison with three of four columns invites somebody to read the three.
