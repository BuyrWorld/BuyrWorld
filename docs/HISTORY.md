...
*Moved out of `IMPLEMENTATION-STATUS.md` on 18 September 2026. The 15
September audit asked for exactly this: "Historical sections of
IMPLEMENTATION-STATUS.md describe features as not built before later sections
mark them done… consolidate that history in a separate documentation pass."*

*Keeping the diary under the status table made the ordering do the work, and
ordering is a weak guarantee — a reader who searches the file still lands in
the middle of a sentence from three weeks ago and reads it as now. So the
record lives here, where every line of it is history by construction, and
`IMPLEMENTATION-STATUS.md` is a status document with no diary in it.*

**Nothing below has been edited to agree with today.** An entry that says
something is not built is reporting the state on the day it was written.
